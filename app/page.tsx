...
export default function OrderPage() {
  ...
  const deliveryFee = subtotal > 0 ? 1.00 : 0
  ...
  // ─── SHARED: Cart panel (used in desktop sidebar + mobile cart view) ──────
  const CartPanel = ({ showCheckoutForm }: { showCheckoutForm?: boolean }) => (
      ...
      <div className="flex-1 overflow-y-auto">
        {cart.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full py-16 text-center px-6">
            ...
            <div className="mt-5 w-full border-t border-dashed border-gray-100 pt-5 space-y-2 text-left">
              {['Salted Honey', 'Salted Caramel', 'Vanilla', 'White Mocha'].map(name => (
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="bg-white rounded-xl p-3 shadow-sm space-y-2">
              ...
              <div className="flex justify-between text-gray-500 text-sm"><span>Delivery fee</span><span>${deliveryFee.toFixed(2)}</span></div>
        ...
        {/* Checkout form — shown inline on desktop or on checkout mobile view */}
        {showCheckoutForm && cart.length > 0 && (
          <form onSubmit={handleSubmit} className="px-4 pb-4 space-y-3">
            ...
            {[
              ...
              { label: 'Address', key: 'address', type: 'text', placeholder: '123 Main St, City, TX', required: true },
            ].map(f => (
              <div key={f.key}>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {[{ value: 'cashapp', label: 'CashApp' }, { value: 'venmo', label: 'Venmo' }, { value: 'in_person', label: 'In Person' }].map(opt => (
            ...
            <button type="submit" disabled={submitting}
              className="w-full bg-amber-800 text-white py-3 rounded-xl font-bold text-sm disabled:opacity-50">
    ...
    <div className="hidden lg:flex min-h-screen">
      ...
      <div className="w-80 bg-white border-l border-gray-100 flex flex-col sticky top-0 h-screen flex-shrink-0">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-white">
          ...
          {cartCount > 0 && (
            <span className="bg-amber-800 text-white text-xs font-bold px-2.5 py-1 rounded-full">{cartCount} item{cartCount !== 1 ? 's' : ''}</span>
          )}
        </div>
        <div className="flex-1 overflow-y-auto">
          <CartPanel showCheckoutForm={true} />
        </div>
      </div>
    </div>
  )
    ...
    <main className="min-h-screen bg-gray-50 relative lg:hidden">
      <div className="absolute top-4 right-4 z-20">
        <button onClick={() => setMobileView('cart')}
          className="relative w-11 h-11 flex items-center justify-center rounded-full bg-white shadow-md active:scale-95 transition-transform">
          <svg className="w-6 h-6 text-amber-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
          </svg>
          {cartCount > 0 && (
            <span className="absolute -top-1 -right-1 bg-amber-800 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{cartCount}</span>
          )}
        </button>
      </div>
      <MenuContent />
      {cartCount > 0 && (
        <div className="fixed bottom-6 left-4 right-4 z-20">
          <button onClick={() => setMobileView('cart')}
            className="w-full bg-amber-800 text-white py-4 rounded-2xl font-bold text-base shadow-lg flex items-center justify-between px-5 active:scale-95 transition-transform">
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-6 left-4 right-4 z-20">
          <button onClick={() => setMobileView('checkout')}
            className="w-full bg-amber-800 text-white py-4 rounded-2xl font-bold text-base shadow-lg active:scale-95 transition-transform">
    ...
    <main className="min-h-screen bg-gray-50 lg:hidden">
      ...
      <form onSubmit={handleSubmit} className="px-4 py-4 pb-40 space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4">
          ...
          <select value={form.delivery_date} onChange={e => setForm(prev => ({ ...prev, delivery_date: e.target.value }))}
            className="w-full border border-gray-200 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-amber-400 bg-white">
            {dates.map(d => <option key={d} value={d}>{formatDate(d)}</option>)}
          </select>
        </div>
        <div className="bg-white rounded-2xl shadow-sm p-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{ value: 'cashapp', label: 'CashApp' }, { value: 'venmo', label: 'Venmo' }, { value: 'in_person', label: 'In Person' }].map(opt => (
              <button key={opt.value} type="button"
                onClick={() => setForm(prev => ({ ...prev, payment_method: opt.value }))}
                className={`py-3 rounded-xl font-semibold text-sm border-2 transition-colors ${form.payment_method === opt.value ? 'bg-amber-800 text-white border-amber-800' : 'bg-white text-gray-700 border-gray-200'}`}>
                {opt.label}
              </button>
            ))}
          </div>
      ...
      <div className="fixed bottom-6 left-4 right-4 z-20">
        <button onClick={handleSubmit} disabled={submitting}
          className="w-full bg-amber-800 text-white py-4 rounded-2xl font-bold text-base shadow-lg disabled:opacity-50 active:scale-95 transition-transform">
          {submitting ? 'Placing order...' : `Place Order · $${total.toFixed(2)}`}
        </button>
      </div>
    </main>
  ...
  return (
    <>
      ...
      {mobileView === 'cart' && <MobileCart />}
      {mobileView === 'checkout' && <MobileCheckout />}
    </>
  )
}
